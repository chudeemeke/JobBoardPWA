...
const urlsToCache = [
    '/JobBoardPWA/',
    '/JobBoardPWA/index.html',
    '/JobBoardPWA/manifest.json'
];
...
self.addEventListener('fetch', event => {
        ...
        event.respondWith(
            fetch(request)
                ...
                .catch(() => {
                    ...
                    return caches.match(request) || caches.match('/JobBoardPWA/index.html');
                })
        );
        return;
    }
    ...
    event.respondWith(
        caches.match(request)
            ...
            .catch(() => {
                // Offline fallback
                return caches.match('/JobBoardPWA/index.html');
            })
    );
});
...
self.addEventListener('notificationclick', event => {
    ...
    if (event.action === 'view') {
        event.waitUntil(
            clients.openWindow('/JobBoardPWA/')
        );
    }
});
